...
export function UniversityHomePage() {
  ...
  const mobileRecentStudents = recentStudents.slice(0, MOBILE_RECENT_STUDENTS);
  const mobileRecentTeachers = recentTeachers.slice(0, MOBILE_RECENT_TEACHERS);
  const mobileRecentCampuses = institutionProfile.campuses.slice(
    0,
    MOBILE_RECENT_CAMPUSES,
  );
  const adminFullName = useMemo(() => {
    const profileFullName =
      `${institution.adminFirstName} ${institution.adminLastName}`.trim();
              ...
              <div className="flex min-w-0 flex-wrap items-center gap-1 sm:gap-1.5">
                ...
                <span className="inline-flex min-w-0 items-center gap-1 rounded-full bg-white/12 px-1.5 py-0.5 text-[0.65rem] font-semibold text-white/88 sm:px-2 sm:py-1 sm:text-[0.75rem]">
                  ...
                  <span className="max-w-[8.5rem] truncate sm:max-w-[12rem] xl:max-w-[14rem]">
                    {[
                      institutionProfile.mainCity,
                      institutionProfile.mainLocality,
                    ]
                      .filter(Boolean)
                      .join(' · ') || 'Sin ubicacion principal'}
                  </span>
                </span>
              </div>
            </div>
              ...
              <div className="space-y-2 sm:space-y-3">
                ...
                <div className="space-y-1.5 sm:hidden">
                  ...
                  {mobileRecentStudents.length > 0 ? (
                    ...
                    <div className="rounded-[0.95rem] border border-dashed border-slate-200 bg-slate-50 px-3 py-3 text-[0.78rem] text-ink-muted">
                ...
                <div className="space-y-2 sm:hidden">
                  <div className="space-y-1.5">
                    ...
                    {mobileRecentTeachers.length > 0 ? (
                      ...
                      <div className="rounded-[0.95rem] border border-dashed border-slate-200 bg-slate-50 px-3 py-3 text-[0.78rem] text-ink-muted">
                        No hay docentes registrados recientemente.
                      </div>
                    )}
                  </div>
                  <div className="space-y-1.5">
                    ...
                    {mobileRecentCampuses.length > 0 ? (
                      ...
                      <div className="rounded-[0.95rem] border border-dashed border-slate-200 bg-slate-50 px-3 py-3 text-[0.78rem] text-ink-muted">
                        No hay sedes registradas recientemente.
                      </div>
                    )}
                  </div>
                </div>

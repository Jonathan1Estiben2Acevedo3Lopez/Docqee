...
export function UniversityTeachersPage() {
  ...
  const paginatedTeachers = useMemo(
    () => filteredTeachers.slice(pageStartIndex, pageStartIndex + rowsPerPage),
    [filteredTeachers, pageStartIndex, rowsPerPage],
  );
  const pageStartLabel = filteredTeachers.length > 0 ? pageStartIndex + 1 : 0;
  const pageEndLabel = Math.min(
    pageStartIndex + paginatedTeachers.length,
  ...
  }, [filteredTeachers.length, pageStartIndex, paginatedTeachers.length]);
          ...
          <>
                ...
                <table className="w-full table-fixed">
                  ...
                  <thead className="sticky top-0 z-10 bg-slate-100 text-left">
                    <tr className="text-[0.6rem] font-bold uppercase tracking-[0.14em] text-ink-muted sm:text-[0.64rem] sm:tracking-[0.16em]">
                      <th className="px-2.5 py-2 sm:px-4 sm:py-2.5">Docente</th>
                  ...
                  >
                    {paginatedTeachers.map((teacher, index) => {
                      const isLast = index === paginatedTeachers.length - 1;
            ...
            <div className="flex shrink-0 flex-col gap-2 border-t border-slate-200/80 bg-white px-3 py-2.5 text-[0.72rem] font-semibold text-ink-muted sm:flex-row sm:items-center sm:justify-between sm:px-5 sm:text-[0.8rem]">
              <p className="text-center sm:text-left">
                ...
                {filteredTeachers.length} · Página {clampedCurrentPage} de{' '}
                {totalPages}
              </p>
